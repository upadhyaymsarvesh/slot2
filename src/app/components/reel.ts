...
export class Reel extends PIXI.Container {
    ...
    private static readonly visibleTiles: Array<number> = [2, 3, 3, 3, 2];
    ...
    constructor(width: number, height: number, id: number, symCount: number) {
        super();

        this.realHeight = height;
        this.tileHeight = 203; // 3 visible tiles
        this.id = id;

        // mask
        const rectMask: PIXI.Graphics = new PIXI.Graphics();
        rectMask.beginFill(0);
        rectMask.drawRect(0, 10, width, height);
        rectMask.endFill();
        ...
        this.tiles = [];
        for (let i: number = 0; i < symCount; i++) {
